    ...
    <section class="event-all" :class="{'event-all-show':isShow}">
        <div class="table-box">
            <div class="edit-input" :class="{'edit-input-show':is_edit}">
                <!-- ref，$refs 用在普通元素上，通过this.ref.name获取dom元素（共有三种用法） -->
                <input @keyup.enter="editData" type="text" placeholder="编辑数据的输入框" v-model="info.content" ref="content">
                ...
                <tbody>
                    <tr v-for="(value,index) in notepad">
                        ...
                        <td>
                            ...
                            <button @click="showDialog(index,value.id)" class="del-btn">删除</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script>
    export default{
            ...
            notepad(){
                ...
                return self.$store.getters.getEventList.filter(function(item){
                    ...
                    if(self.screen_type !== 0 && self.screen_title === ''){
                        ...
                        //状态和关键词两个都不为空
                        if(self.screen_type === item.type && item.content.indexOf(self.screen_type) != -1){
                            return item;
                        }
                    }else if(self.screen_type === 0 && self.screen_title !== ''){
                        // 有关键词没状态
        ...
        methods:{
            // 切换下拉框
            doSelect(type){
                this.screen_type = type; //改变筛选框状态
                this.active = false; //选完把筛选框隐藏
            },
            // 根据状态判断类型
            getType(type){
                let src = '';
